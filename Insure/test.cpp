...
QString Test::startTest()
{


    redis = new QtRedis("192.168.56.1",6379);
  ...
  qDebug() << "AUTH:" << redis->auth("redis");

    // Set and Get example
    QString str_in;
    str_in="\"903409oijfkdsjfk\"";


    qDebug() << "SET:" << redis->set("key", str_in);
...
/*
   ...
   QString str2(reply.value.toString());
    qDebug() << "RAW中文:" << "("<< reply.type << ")" << str2;
    qDebug() << QTextCodec::codecForName("GBK")->toUnicode(str2);
